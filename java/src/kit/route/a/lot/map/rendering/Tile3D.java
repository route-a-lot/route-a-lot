...
public class Tile3D extends Tile {
    ...
    protected void reset() {
        super.reset();
        heightdata = new float[HEIGHT_RESOLUTION][HEIGHT_RESOLUTION];  
    }
    ...
    public void render(GL gl) {
        ...
        float stepSize = (getBottomRight().getLatitude() - topLeft.getLatitude()) / HEIGHT_RESOLUTION;
        Coordinates pos = new Coordinates();
        for (int x = 0; x < HEIGHT_RESOLUTION - 1; x++) {
            ...
            for (int y = 0; y < HEIGHT_RESOLUTION; y++) {
                ...
                float color = Util.mapFloat(h, 0, 127, 0.3f, 1);
                gl.glColor3f(color, color, color);
                gl.glTexCoord2f(x / HEIGHT_RESOLUTION, y / HEIGHT_RESOLUTION);
                ...
                gl.glColor3f(color, color, color);
                gl.glTexCoord2f((x + 1) / HEIGHT_RESOLUTION, y / HEIGHT_RESOLUTION) ;
                gl.glVertex3f(pos.getLongitude(), pos.getLatitude(), h);
            }
            gl.glEnd();
        }
